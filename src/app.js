(function () {
  ...
  var domain = window.SeddleUpDomain;
  if (!domain) {
    throw new Error("SeddleUpDomain was not loaded.");
  }
  var categories = ["Transportation", "Food", "Lodging", "Activities", "Supplies", "Other"];
  var expenseStatuses = domain.expenseStatuses;
  var summarizeTrip = domain.summarizeTrip;
  var calculateEqualShares = domain.calculateEqualShares;
  var filterExpenses = domain.filterExpenses;
  var statusCounts = domain.statusCounts;
  var formatCurrency = domain.formatCurrency;
  var formatDate = domain.formatDate;
  var roundCurrency = domain.roundCurrency;
  var escapeHtml = domain.escapeHtml;
  var escapeAttr = domain.escapeAttr;
  var currentUser = {
    id: "user-mia",
    name: "Mia Donovan",
  ...
  function renderTripDetail(trip, route) {
    ...
    var filteredExpenses = filterExpenses(trip.expenses, filter, currentUser.id);
